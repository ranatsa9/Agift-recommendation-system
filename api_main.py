...
import exposure  # noqa: F401 - required to resolve the v4 pickled artifact
import text_block  # noqa: F401 - required to resolve the v4 text feature block


DEFAULT_ARTIFACT_PATH = Path(__file__).resolve().parent / "models" / "gift_recommender_v4.joblib"
...
def metrics() -> dict[str, object]:
    """Expose v4 evaluation and dual-space model metadata."""
    return {
        "precision_at_5": META.get("precision_at_5"),
        "ndcg_at_10": META.get("ndcg_at_10"),
        "n_items": META.get("n_items", len(ENGINE.catalog)),
        "n_features_ranking": META.get("n_features_ranking", META.get("n_features")),
        "n_features_clustering": META.get(
            "n_features_structural", META.get("n_features_clustering")
        ),
        "clusters": META.get("k"),
