...
def resolve_api_url() -> str:
    ...
    return str(
        ...
        or "https://agift-recommendation-system-eta.vercel.app"
    ).strip().rstrip("/")


DEFAULT_API_URL = resolve_api_url()
DEFAULT_MODEL_PATH = os.getenv(
    "GIFT_MODEL_PATH", "models/gift_recommender_v4.joblib"
)
...
st.markdown(
    ...
    <style>
      ...
      .hero h1 { position:relative; z-index:1; font-size:clamp(2.7rem,5vw,4.9rem); line-height:1.01;
                 margin:.65rem 0 1rem; max-width:690px; letter-spacing:-.035em; }
      ...
      div[data-baseweb="menu"] { background:#fffafb !important; border:1px solid var(--line) !important;
              box-shadow:0 18px 45px rgba(91,55,70,.16) !important; }
      div[data-baseweb="menu"] li, div[data-baseweb="menu"] li * {
              color:var(--ink) !important; font-weight:600 !important; opacity:1 !important;
              filter:none !important; text-shadow:none !important; transform:none !important; }
      div[data-baseweb="menu"] li:hover { background:var(--surface-2) !important; }
      ...
      .compare-row:not(.compare-head):hover { background:rgba(185,95,132,.07); }
      .compare-head { background:rgba(185,95,132,.09); color:var(--muted);
      ...
      .match-track { height:6px; border-radius:99px; overflow:hidden; background:#eadde3; }
      ...
      section[data-testid="stSidebar"] { background:rgba(245,237,241,.96);
...
BLOCKED_PRODUCT_NAMES = {"mad smartphone tycoon"}


def sanitize_recommendations(
    recommendations: list[dict[str, Any]], query: dict[str, Any]
) -> list[dict[str, Any]]:
    """Remove known catalog errors and stop digital games leaking into other interests."""
    selected = set(query.get("specific_interests") or [])
    gaming_selected = "PC & Console Gaming" in selected
    cleaned: list[dict[str, Any]] = []
    for item in recommendations:
        name = str(first_present(item, ["product_name", "name", "title"]) or "").strip()
        if name.casefold() in BLOCKED_PRODUCT_NAMES:
            continue
        searchable = " ".join(
            str(first_present(item, [field]) or "")
            for field in ("product_name", "name", "title", "brand", "category", "sub_category")
        ).casefold()
        is_digital_game = any(
            marker in searchable
            for marker in ("pc games", "pc game", "steam", "video game", "digital game")
        )
        if selected and not gaming_selected and is_digital_game:
            continue
        cleaned.append(item)
    return cleaned
...
def load_options(api_url: str) -> tuple[dict[str, list[str]], bool, str]:
    try:
        ...
        response = requests.get(
            f"{api_url}/options",
            timeout=30,
            headers={"Accept": "application/json", "User-Agent": "Giftly-Streamlit/1.0"},
        )
        response.raise_for_status()
        if not response.content.strip():
            raise ValueError("The options endpoint returned an empty response")
        data = response.json()
        return {
        ...
        # Options are UI metadata. A healthy recommendation API can safely use
        # the interface's built-in list if Vercel returns an empty cold-start response.
        try:
            health = requests.get(
                f"{api_url}/health",
                timeout=30,
            ...
            if health.json().get("status") == "ok":
                return DEFAULT_OPTIONS, True, f"Using built-in options ({exc})"
...
def generate_recommendations(
    ...
    recommendations = sanitize_recommendations(recommendations, query)
...
with st.sidebar:
    ...
    api_url = st.text_input(
        ...
        help="Enter the public Vercel API address without /health or /options.",
    ).strip().rstrip("/")
...
if page == "◈ Model Insights":
    if artifact is None:
        ...
        n_items = int(model_meta.get("n_items", 0))
        n_features = int(model_meta.get("n_features", 0))
        n_segments = int(model_meta.get("k", 0))
        ndcg_at_10 = model_meta.get("ndcg_at_10")
        quality_value = f"{float(ndcg_at_10):.3f}" if ndcg_at_10 is not None else "N/A"
        st.markdown(
            f"""
            <div class="insight-grid">
              <div class="insight-card"><small>Product catalog</small><strong>{n_items:,}</strong><span>real gifts analysed</span></div>
              <div class="insight-card"><small>Feature space</small><strong>{n_features}</strong><span>signals per product</span></div>
              <div class="insight-card"><small>Learned segments</small><strong>{n_segments}</strong><span>distinct gift profiles</span></div>
              <div class="insight-card"><small>NDCG@10</small><strong>{quality_value}</strong><span>ranking quality</span></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
# Clean previously cached/session-stored results too, so a blocked product
# disappears immediately after this Streamlit version is deployed.
if recommendations:
    recommendations = sanitize_recommendations(
        recommendations,
        st.session_state.get("last_gift_query", {}),
    )
    st.session_state["gift_results"] = recommendations
